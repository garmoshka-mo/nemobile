...
var googleAnalytics = {
    ...
    setLookForChat: function(lookupInProgress) {
        this.didTry = true;
        if(lookupInProgress) {
            this.lookupStartTime = Date.now();
        }
        else {
            this.awaitDuration = (Date.now() - this.lookupStartTime) / 1000;
        }
    ...
    boredToWait: function () {
        this.lookupInProgress = false;
        this.setLookForChat(false);
        this.setMetric(2, 0);
        this.setMetric(3, this.awaitDuration);
        this.setDimensions();
        this.event('page', 'boredToWait');
    },
    dialogStart: function(){
        this.dialogStartTime = Date.now();
        this.didTry = true;
        this.chatInProgress = true;
        this.chat20minutesInterval = this.chat20minutes();
    },
    dialogComplete: function () {
        this.dialogDuration = (Date.now() - this.dialogStartTime) / 1000;
        if(this.chat20minutesInterval) {
            clearInterval(this.chat20minutesInterval);
        }
        this.chatInProgress = false;
        this.setMetric(2, this.dialogDuration);
        this.setMetric(3, this.awaitDuration);
        this.setDimensions();
        this.event('dialog', 'complete');
    },
    chat20minutes: function() {
        return setInterval(function() {
            googleAnalytics.event('dialog', 'chat20minutes');
        }, 1200000);
    },

    setMetric: function(number, value) {
        if (RAN_AS_APP){
            this.executeMobile(function(){
    ...
    setDimension: function(number, value) {
        if (RAN_AS_APP){
            this.executeMobile(function(){
...
$(window).bind('beforeunload', function() {
    if(googleAnalytics.lookupInProgress) {
        googleAnalytics.boredToWait();
    }
    else if(googleAnalytics.chatInProgress) {
        googleAnalytics.dialogComplete();
    }
    else {
        if(!this.didTry)
            this.event('page', 'didntTry');
    }
});